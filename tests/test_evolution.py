...
async def setup_db(require_db):
    ...
    await db.execute(
        "DELETE FROM evolution_queue WHERE target_component = 'analyze_data'"
    )
    await db.execute(
        "DELETE FROM system_model WHERE component_type = 'skill' AND component_name LIKE 'gen-dummy%'"
    )
    await db.execute(
        "DELETE FROM system_model WHERE component_type = 'skill' AND component_name LIKE 'gen-integration-dummy%'"
    )
    yield
...
"""
    ...
    patterns = await db.fetch(
        "SELECT * FROM reference_patterns WHERE source_repo = 'test_ref'"
    )
